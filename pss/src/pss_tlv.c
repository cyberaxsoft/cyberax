...
gchar* return_message_type_name(MessageType type)
{
	...
	switch(type)
	{
		case mt_Undefined:	result = strdup("Undefined"); break;
		case mt_Request:	result = strdup("Request"); break;
		case mt_Reply:		result = strdup("Reply"); break;
		case mt_Ack:		result = strdup("Ack"); break;
		case mt_Nak:		result = strdup("Nak"); break;
		case mt_Event:		result = strdup("Event"); break;
		case mt_Eot:		result = strdup("Eot"); break;
		default:			result = strdup("Undefined"); break;
	}
...
const gchar* server_command_to_str(HardwareServerCommand command)
{
	switch(command)
	{
		...
		case hsc_DCUpdatePrices: 			return "DCUpdatePrices";
