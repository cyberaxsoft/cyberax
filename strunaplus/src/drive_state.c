...
StrunaPlusRxState			rx_state;
StrunaPlusExchangeState		exchange_state;
gboolean					exchange_complete;
...
void safe_increment_active_tank_index()
{
	...
	{
		safe_set_exchange_complete(TRUE);
		safe_set_exchange_state(es_Idle);
	}
	else
	{
...
StrunaPlusRxState safe_get_rx_state()
{
	StrunaPlusRxState result = rs_None;
...
void safe_set_rx_state(StrunaPlusRxState new_value)
{
	g_mutex_lock(&driver_mutex);
...
StrunaPlusExchangeState safe_get_exchange_state()
{
	StrunaPlusExchangeState result = es_Idle;
...
void safe_set_exchange_state(StrunaPlusExchangeState new_value)
{
	g_mutex_lock(&driver_mutex);
...
gboolean safe_get_exchange_complete()
{
	gboolean result = FALSE;

	g_mutex_lock(&driver_mutex);

	result = exchange_complete;
...
void safe_set_exchange_complete(gboolean new_value)
{
	g_mutex_lock(&driver_mutex);

	exchange_complete = new_value;
