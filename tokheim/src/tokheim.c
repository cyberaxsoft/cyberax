			...
			case  ct_Uart:
				{
					...
					if (port!=NULL)
					{
						...
						if (uart_descriptor !=-1)
						{
						    ...
						    read_thread = g_thread_new("tokheim_read_thread", read_thread_func, NULL);
						    if (read_thread == NULL)
						    {
			...
			case  ct_TcpIp:
				{
				    ...
				    if (read_thread == NULL)
				    {
				    	add_log(TRUE, TRUE, log_options.trace, log_options.system, "Tokheim error starting read thread");
						safe_set_status(drs_ErrorConnection);
						return de_ErrorConnection;
				    }
				    else
				    {
				    	add_log(TRUE, TRUE, log_options.trace, log_options.system, "Tokheim read thread started");
				    	read_thread->priority = G_THREAD_PRIORITY_LOW;
				    }
				    ...
				    if (write_thread == NULL)
				    {
				    	add_log(TRUE, TRUE, log_options.trace, log_options.system, "Tokheim error starting write thread");
						safe_set_status(drs_ErrorConnection);
						return de_ErrorConnection;
				    }
				    else
				    {
				    	add_log(TRUE, TRUE, log_options.trace, log_options.system, "Tokheim write thread started");
				    	read_thread->priority = G_THREAD_PRIORITY_LOW;
				    }
				}
				break;
