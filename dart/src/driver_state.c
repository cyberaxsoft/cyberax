...
void safe_set_driver_state_from_settings(LibConfig* configuration)
{
	...
	if (dispencer_count > 0)
	{
		for (guint8 i = 0; i < dispencer_count; i++)
		{
			...
			dispencers[i].original_state = dds_NotProgrammed;
			dispencers[i].original_nozzle_state = FALSE;
			dispencers[i].original_active_grade = 0;

			dispencers[i].exchange_mode = dem_Pooling;
			dispencers[i].exchange_state = des_Free;

			dispencers[i].block_sequence_number = 0;
			dispencers[i].reply_block_sequence_number = 0;
			...
			dispencers[i].sending_error_counter = 0;
			dispencers[i].uart_error_counter = 0;
...
void safe_set_block_sequence_number(guint8 disp_index, guint8 new_value)
{
	...
	{
		dispencers[disp_index].block_sequence_number = new_value;
	}
	g_mutex_unlock(&driver_mutex);
}

void safe_increment_block_sequence_number(guint8 disp_index)
{
	g_mutex_lock(&driver_mutex);

	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].block_sequence_number++;
		if (dispencers[disp_index].block_sequence_number >= 0x0F)
		{
			dispencers[disp_index].block_sequence_number = 0;
		}
	}
	g_mutex_unlock(&driver_mutex);
}
...
guint8 safe_get_block_sequence_number(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].block_sequence_number;
	}
...
void safe_set_reply_block_sequence_number(guint8 disp_index, guint8 new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].reply_block_sequence_number = new_value;
	}
	g_mutex_unlock(&driver_mutex);
}
...
guint8 safe_get_reply_block_sequence_number(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].reply_block_sequence_number;
	}
...
void safe_set_last_sended_disp_index(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		last_sended_disp_index = disp_index;
	}
...
{
	DartExchangeState result = des_Free;
...
{
	DartExchangeMode result = dem_Pooling;

	g_mutex_lock(&driver_mutex);

	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].exchange_mode;
	}
...
void safe_set_exchange_mode(guint8 disp_index, DartExchangeMode new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].exchange_mode = new_value;
	}
...
gboolean safe_get_original_nozzle_state(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].original_nozzle_state;
	}
...
void safe_set_original_nozzle_state(guint8 disp_index, gboolean new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].original_nozzle_state = new_value;
	}
...
{
	DartDispencerState result = dds_NotProgrammed;
...
guint8 safe_get_uart_error_counter(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].uart_error_counter;
	}
...
void safe_set_uart_error_counter(guint8 disp_index, guint8 new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].uart_error_counter = new_value;
	}
...
void safe_increment_uart_error_counter(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].uart_error_counter++;
	}
...
guint8 safe_get_sending_error_counter(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].sending_error_counter;
	}
...
void safe_set_sending_error_counter(guint8 disp_index, guint8 new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].sending_error_counter = new_value;
	}
...
void safe_increment_sending_error_counter(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].sending_error_counter++;
	}
...
guint8 safe_get_original_active_grade(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		result = dispencers[disp_index].original_active_grade;
	}
...
void safe_set_original_active_grade(guint8 disp_index, guint8 new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].original_active_grade = new_value;
	}
...
void safe_disp_clear(guint8 disp_index)
{
	...
	if (disp_index  < dispencer_count)
	{
		...
		dispencers[disp_index].exchange_state = des_Free;
