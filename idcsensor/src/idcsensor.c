...
DriverStatus 	status = ds_NotInitializeDriver;
GMutex			status_mutex;
...
{
	DriverStatus result = ds_UndefinedError;
...
//----------------------------------------         safe_get_sensor_data_by_num          -----------------------------------------------------------

DriverError safe_get_sensor_data_by_num(guint32 sensor_num, SensorParams* params, gboolean* online)
{
	guint8 sensor_index = 0;

	DriverError result = safe_get_sensor_index_by_num(sensor_num, &sensor_index);

	if (result == de_NoError)
	{
		result = safe_get_sensor_data(sensor_index, params, online);
	}
...
ssize_t read_func(guint8* buffer)
{
	...
	{
		case ct_Uart:
			result = read(uart_descriptor,buffer, BUFFER_SIZE);
			break;
...
guchar init_lib(LibConfig config)
{
	...
	if (!safe_get_driver_init())
	{
		...
		if (log_options.dir != NULL )
		{
			if (log_options.enable)
			{
				if (!init_log_dir(log_options.dir, LOG_PREFIX))
				{
					...
					safe_set_status(ds_ErrorLogging);
		...
		{
			safe_set_status(ds_ErrorConfiguration);
...
guchar close_lib()
{
	if (safe_get_driver_init())
	{
		...
		safe_set_status(ds_NotInitializeDriver);
...
guchar get_sensor_count(guint8* count)
{
	if (!safe_get_driver_init())
	{
		return de_NotInitializeDriver;
	...
	*count = safe_get_sensor_count();

	add_log(TRUE, TRUE, log_options.trace, log_options.requests, "Get sensor count %d", *count);
...
guchar get_sensor_info(guint8 index_sensor, SensorConf* sensor_conf)
{
	if (!safe_get_driver_init())
	{
		return de_NotInitializeDriver;
	...
	DriverError result = safe_get_sensor_info(index_sensor, sensor_conf);

	if (result == de_NoError)
	{
		add_log(TRUE, TRUE, log_options.trace, log_options.requests, "Get sensor info i:%d num:%d addr:%d", index_sensor, sensor_conf->num, sensor_conf->addr);
	}
	else
	{
...
guchar get_sensor_state(guint32 sensor_num, SensorParams* params, gboolean* online)
{
	if (!safe_get_driver_init())
	{
		return de_NotInitializeDriver;
	...
	guchar result = safe_get_sensor_data_by_num(sensor_num, params, online);

	if (result == de_NoError)
	{
		add_log(TRUE, TRUE, log_options.trace, log_options.requests, "Get sensor %d state return online = %d", sensor_num, *online);
	}
	else
	{
		add_log(TRUE, TRUE, log_options.trace, log_options.requests, "Get sensor %d state return error %d", sensor_num, result);
	}
