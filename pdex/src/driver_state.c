...
void safe_set_driver_state_from_settings(LibConfig* configuration)
{
	...
	if (dispencer_count > 0)
	{
		for (guint8 i = 0; i < dispencer_count; i++)
		{
			...
			dispencers[i].char_ready = ctrl_ENQ;
...
void safe_set_command_sended(guint8 disp_index, gboolean new_value)
{
	...
	{
		dispencers[disp_index].command_sended = new_value;
	}
...
gboolean safe_get_command_sended(guint8 disp_index)
{
	...
	result = dispencers[disp_index].command_sended;
...
void safe_set_char_ready(guint8 disp_index, PdexCtrlChar new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		dispencers[disp_index].char_ready = new_value;
	}
...
{
	PdexCtrlChar result = ctrl_Undefined;

	g_mutex_lock(&driver_mutex);

	result = dispencers[disp_index].char_ready;
...
void safe_set_auth_code(guint8 disp_index, guint8* new_value)
{
	...
	if (disp_index  < dispencer_count)
	{
		memcpy(dispencers[disp_index].authorization_code, new_value, TRK_AUTH_CODE_LENGTH);
	}
...
void safe_get_auth_code(guint8 disp_index, guint8* buffer)
{
	...
	memcpy(buffer, dispencers[disp_index].authorization_code, TRK_AUTH_CODE_LENGTH);
